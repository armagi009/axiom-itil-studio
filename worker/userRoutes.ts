...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
    ...
    app.get('/api/agents', async (c) => {
        try {
            ...
            return c.json({ success: false, error: 'Failed to retrieve agents' }, { status: 500 });
        }
    });
    /**
    ...
    app.put('/api/agents/:id', async (c) => {
        try {
            ...
            const config = await c.req.json<AgentConfig>();
            // Basic validation
            if (!config || !config.persona || !config.playbook) {
                return c.json({ success: false, error: 'Invalid configuration data' }, { status: 400 });
            }
            const controller = getAppController(c.env);
            const updatedAgent = await controller.updateAgentConfig(agentId, config);
            if (!updatedAgent) {
                return c.json({ success: false, error: 'Agent not found' }, { status: 404 });
            }
            return c.json({ success: true, data: updatedAgent });
        } catch (error) {
            console.error(`Failed to update agent ${c.req.param('id')}:`, error);
            return c.json({ success: false, error: 'Failed to update agent configuration' }, { status: 500 });
        }
    });
}